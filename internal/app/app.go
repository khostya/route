...
func (a *App) Start(ctx context.Context, n int, result chan<- error) error {
...
func (a *App) worker(ctx context.Context, n int, result chan<- error) {
	defer a.wg.Done()
	...
	for {
		...
		case <-a.startWorker:
			go a.worker(ctx, rand.Intn(math.MaxInt), result)
			a.wg.Add(1)
		case <-a.stopWorker:
			return
		}
